...
void matmul_prefetch(const float* A, const float* B, float* C,
    ...
    const int PF_DIST = 128; 
    const int PF_LEVEL = _MM_HINT_T0; 
    ...
    for (int i = 0; i < M; ++i) {
        for (int j = 0; j < N; ++j) {
            ...
            int pf_safe_limit = K - PF_DIST;

            for (; p + 63 < pf_safe_limit; p += 64) {

                _mm_prefetch((const char*)(b + p + PF_DIST),      _MM_HINT_T0);
                _mm_prefetch((const char*)(b + p + PF_DIST + 16),_MM_HINT_T0);
                _mm_prefetch((const char*)(b + p + PF_DIST + 32),_MM_HINT_T0);
                _mm_prefetch((const char*)(b + p + PF_DIST + 48), _MM_HINT_T0);
